...
- `reconnect.go`'s `handleVPNSwitch` is the only asynchronous action: it answers `202` and runs `vpn.Manager.Switch` in a goroutine, marking the target resumed in `PausedTracker` immediately and the VPNs the switch paused once it has paused them — so persistence stays hooked here like for the plain pause/resume handlers.
