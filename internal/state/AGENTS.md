...
- **Where a new field belongs — `config.yaml` vs. `state.json` vs. nowhere** — decide by asking what the value *is*, not just whether it needs to survive a restart:
  ...
  - **Measured observations** ("what happened before, learned from"): the VPN session history (`vpn-sessions.jsonl`, owned by `internal/vpn/history.go`) is the only current example. Not policy, not intent, and — unlike computed status — it can't be recomputed at startup, yet it's machine-specific and disposable. It lives in the OS cache dir next to the PID file (never next to `config.yaml`), is bounded (500 records / 30 days), survives clean shutdown (`Manager.Remove` doesn't touch it), and deleting it only resets what was learned. Same never-return-errors contract as `PausedTracker`.
