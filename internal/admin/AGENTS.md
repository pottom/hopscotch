...
- `notifications.go`'s `handlePutNotifications` follows the exact same shape as `rules.go`'s `handleRules`: decode → mutate `cfg.Notifications` under `cfgMu` → `config.WriteConfig` → apply live (`s.notifyCtl.SetConfig`, instead of a router/manager call). `/status`'s `Notifications` field (`notificationsJSON(s.notifyCtl.Config())`) is how both UIs read the current value to render the Settings tab — keep both in sync if `config.NotificationsConfig` grows a field.
