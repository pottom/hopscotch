...
//
// Usage: node capture_web.js <admin-port> <docs-dir> <auth-admin-port>
...
async function main() {
  const [, , adminPort, docsDir, authAdminPort] = process.argv;
  ...
  try {
    ...
    await page.fill('#routes-tester-input', '10.42.0.5'); // exercise the CIDR-block rule live
    ...
    await page.click('.log-chip[data-level="WARN"]'); // filter to WARN+ so cache-warm's retries stand out
    await page.waitForTimeout(500);
    ...
    if (authAdminPort) {
      ...
      await loginPage.goto(`http://127.0.0.1:${authAdminPort}/login`);
      await loginPage.waitForSelector('form[action="/api/login"]');
      await loginPage.screenshot({ path: `${docsDir}/ui-login.png` });
      await loginPage.close();
    }
  } finally {
    // Always close, even if a screenshot step throws — otherwise a failed
    // run leaks a chromium process instead of just failing cleanly.
    await browser.close();
  }
}
