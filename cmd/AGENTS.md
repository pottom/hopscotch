...
- `trust.go` pins *every* host key type the server offers (one handshake per family with `HostKeyAlgorithms` narrowed to ED25519 / ECDSA / RSA-SHA2, aborted in `HostKeyCallback` before auth). Pinning only one type makes OpenSSH clients with a different `HostKeyAlgorithms` preference report "host identification has changed". Compare against known_hosts only through the `knownhosts` callback (hashed entries, patterns, `[host]:port`), never by string search. A same-type pinned key that differs is a refusal: add nothing for that host, return an error, and don't suggest re-running trust.
- `start.go` passes the VPN session history path (`vpn.HistoryFileName` in the same OS cache dir as the PID file, see `internal/state/AGENTS.md`) to `vpn.NewManager`. `vpn.go`'s `hopscotch vpn stats` reads that same file directly via `vpn.ReadReports` — it works without a running daemon and never talks to the admin API.
